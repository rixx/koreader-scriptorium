...
package.preload["rapidjson"] = function()
    return {
        ...
        encode = function(t)
            return t
        end, -- pass tables through; http stub captures them
...
package.preload["docsettings"] = function()
    return {
        ...
        open = function()
            error("DocSettings:open must never be called (destructive)")
        end,
    }
end
...
local function sidecar(status, modified, n_highlights, md5)
    ...
    for i = 1, n_highlights do
        annotations[i] = {
            drawer = "lighten",
            text = "hl " .. i,
            datetime = ("2026-06-%02d 10:00:00"):format(i),
            pageno = i * 10,
        }
    end
    return {
...
table.insert(
    SIDECARS["/books/finished.epub"].annotations,
    { drawer = "lighten", text = "new", datetime = "2026-07-02 09:00:00", pageno = 99 }
)
MTIMES["/books/finished.epub.sdr/metadata.epub.lua"] = 2000
HTTP_RESPONSE = { results = { { md5 = ("a"):rep(32), action = "updated_read", read_id = 1, highlights_stored = 4 } } }
...
HTTP_RESPONSE = { results = { { md5 = ("d"):rep(32), action = "created_book", read_id = 2, highlights_stored = 1 } } }
plugin:scanAndPush({ interactive = true, force = true })
